...
// ?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê
...
app.use((req, res, next) => {
  ...
  console.warn(`?†Ô∏è  CSRF blocked: origin=${origin} host=${host} path=${req.path}`);
...
<label for="rememberMe">ID / Password ?Ä??/label>
</div>
<button type="submit">Sign In</button>
<div class="err" id="err">ID ?êÎäî ÎπÑÎ?Î≤àÌò∏Í∞Ä ?¨Î∞îÎ•¥Ï? ?äÏäµ?àÎã§.</div>
</form>
</div>
<script>
(function(){
...
app.post('/login', (req, res) => {
  ...
  if (!account) {
    console.warn(`?†Ô∏è  Unknown account "${uid}" at ${new Date().toISOString()}`);
    return res.redirect('/login?fail=1');
  }
  ...
  console.warn(`?†Ô∏è  Failed login [${uid}] at ${new Date().toISOString()}`);
...
// ?Ä?Ä Service-Worker-Allowed header for scoped SWs ?Ä?Ä
...
// ?Ä?Ä Static assets BEFORE auth ??JS/CSS/images/fonts served without login ?Ä?Ä
...
app.get('/api/data', async (req, res) => {
  const t0 = Date.now();
  try {
    console.log('\n?ì° SharePoint ?∞Ïù¥???îÏ≤≠ (Table_HQ + Past_History + Asset_List)...');
    ...
    if (results[1].status === 'rejected') console.warn('?†Ô∏è  History fetch failed:', results[1].reason.message);
    if (results[2].status === 'rejected') console.warn('?†Ô∏è  Assets fetch failed:', results[2].reason.message);
...
app.post('/api/report', async (req, res) => {
  ...
  try {
    ...
    console.log(`\n?ìÑ Report: ${MONTHS_EN[month]} ${year} (${action}) lang=${safeLang} type=${safeType} region=${safeRegion}`);
...
app.post('/api/annual-report', async (req, res) => {
  ...
  try {
    ...
    const safeRegion = validRegions.includes(region) ? region : 'global';
    const regionBranches = safeRegion === 'korea' ? KOREA_BRANCHES : GLOBAL_BRANCHES;
    console.log(`\n?ìÑ Annual Report: ${year} (${action}) lang=${safeLang} region=${safeRegion}`);
...
// ?Ä?Ä SERVER START (pm2 managed) ?Ä?Ä
const server = app.listen(PORT, async () => {
  console.log(`\n?? d'strict Error Dashboard v5.6.0 ??http://localhost:${PORT} (PID ${process.pid})`);
  console.log('?ìä SharePoint Excel ?∞Îèô (?òÏù¥ÏßÄ ?ëÏÜç/?àÎ°úÍ≥†Ïπ® ??Î°úÎìú)\n');
  validateConfig();
  try { await getToken(); console.log('??Azure AD ?∏Ï¶ù ?ÑÎ£å\n'); }
  catch(e) { console.error('?†Ô∏è  ?∏Ï¶ù ?§Ìå®:',e.message,'\n   ??AZURE_SECRET???ïÏù∏?òÏÑ∏??n'); }
});

server.on('error', function(err) {
  console.error('???úÎ≤Ñ ?úÏûë ?§Ìå®:', err.code, err.message);
  process.exit(1);
});
