...
function renderP0(){
  ...
  el('zone-hm').innerHTML='<div class="hm-grid">'+sortedZones.map(function(z){var v=zc[z]||0,op=.05+v/maxZ*.85;return'<div class="hm-cell" style="background:rgba(37,99,235,'+op+');color:'+(v>maxZ*.5?'#fff':'#1d4ed8')+'" title="'+esc(z)+'" onclick="filterZone('+JSON.stringify(z).replace(/</g,'\\u003c')+')">'+'<div class="hm-val">'+v+'</div><div class="hm-label">'+z+'</div></div>'}).join('')+'</div>';
