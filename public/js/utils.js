...
/* ?Ä?Ä Account info from cookie (set by server on login) ?Ä?Ä */
...
/* ?Ä?Ä Branch / Region Registry (Frontend) ?Ä?Ä */
...
/* URL-based locale: /kr ??Korea+?úÍ?, /en ??Global+English, / ??Global+English */
...
function catFull(c){var ko=typeof _lang!=='undefined'&&_lang==='ko';return({Software:'<span class="cat-sw">'+(ko?'?åÌîÑ?∏Ïõ®??:'Software')+'</span>',Hardware:'<span class="cat-hw">'+(ko?'?òÎìú?®Ïñ¥':'Hardware')+'</span>',Network:'<span class="cat-net">'+(ko?'?§Ìä∏?åÌÅ¨':'Network')+'</span>'}[c]||esc(c))}
